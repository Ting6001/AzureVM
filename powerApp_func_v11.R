  ...
  if (length(setdiff(df_prj$project_code_old, df_proj$project_code)) != 0){
    ...
    if (sum(dim(df_prj)) == 0){
      ...
      proj_prop <- df_proj %>%
        ...
        mutate(hr_ratio = ifelse(unique(df_prj$execute_hour) %in% c(0, '') | is.na(unique(df_prj$execute_hour)), 1, unique(df_prj$execute_hour) / execute_hour),
               mon_ratio = ifelse(unique(df_prj$execute_month) %in% c(0, '') | is.na(unique(df_prj$execute_month)), 1, unique(df_prj$execute_month) / execute_month))
